...
namespace stdx::details
{
    ...
    protected:
        ...
        explicit constexpr BaseExpectedStorage(std::in_place_index_t<0>, Ts&&... Args) noexcept(NothrowConstructible<T, Ts...>()) :
            Data(std::in_place_index<0>, std::forward<Ts>(Args)...), bHasValue(true)
        {
        }

        template <typename... Ts>
        explicit constexpr BaseExpectedStorage(std::in_place_index_t<1>, Ts&&... Args) noexcept(NothrowConstructible<E, Ts...>()) :
