...
#define _EXPECTED_UNION(TriviallyDestructible, Destructor)                                                                       \
    ...
    union ExpectedUnion<T, E, TriviallyDestructible> {                                                                           \
        ...
        explicit constexpr ExpectedUnion(std::in_place_index_t<0>, Ts&&... Args) noexcept(                                       \
            NothrowConstructible<T, Ts...>::value) :                                                                             \
            Value(std::forward<Ts>(Args)...) {}                                                                                  \
                                                                                                                                 \
        template <typename... Ts>                                                                                                \
        explicit constexpr ExpectedUnion(std::in_place_index_t<1>, Ts&&... Args) noexcept(                                       \
            NothrowConstructible<Unexpected<E>, Ts...>::value) :                                                                 \
