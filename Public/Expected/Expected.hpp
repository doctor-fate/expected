...
namespace stdx
{
    ...
    public:
        ...
        template <typename U, typename _Traits = details::ConstructibleFromU<T, E, U>, typename std::enable_if_t<_Traits::Implicit, int> = 0>
        constexpr Expected(U && Value) noexcept(_Traits::Nothrow) : Super(WithValue, std::forward<U>(Value))
        {
        }

        template <typename U, typename _Traits = details::ConstructibleFromU<T, E, U>, typename std::enable_if_t<_Traits::Explicit, int> = 0>
        constexpr explicit Expected(U && Value) noexcept(_Traits::Nothrow) : Super(WithValue, std::forward<U>(Value))
        {
        }

        template <typename G = E, typename std::enable_if_t<details::Constructible<E, const G&>() && details::Convertible<const G&, E>(), int> = 0>
        constexpr Expected(const Unexpected<G>& Unex) noexcept(details::NothrowConstructible<E, const G&>()) : Super(WithError, Unex.Value())
        {
        }

        template <typename G = E, typename std::enable_if_t<details::Constructible<E, const G&>() && !details::Convertible<const G&, E>(), int> = 0>
        constexpr explicit Expected(const Unexpected<G>& Unex) noexcept(details::NothrowConstructible<E, const G&>()) : Super(WithError, Unex.Value())
        {
        }

        template <typename G = E, typename std::enable_if_t<details::Constructible<E, G&&>() && details::Convertible<G&&, E>(), int> = 0>
        constexpr Expected(Unexpected<G> && Unex) noexcept(details::NothrowConstructible<E, G&&>()) : Super(WithError, std::move(Unex).Value())
        {
        }
        ...
        template <typename U, typename... Ts, typename std::enable_if_t<details::Constructible<E, std::initializer_list<U>&, Ts...>::value, int> = 0>
        ...
        template <
            ...
            typename std::enable_if_t<
                details::And<details::Or<details::IsVoid<T>, details::NothrowConstructible<E, const G&>>, details::MoveAssignable<E>>::value,
                int> = 0>
        Expected& operator=(const Unexpected<G>& Unex) noexcept(details::NothrowConstructible<E, const G&>() && details::NothrowMoveAssignable<E>())
        {
            if (Super::HasValue())
            {
                if constexpr (!details::IsVoid<T>())
        ...
        template <
            typename G = E,
            typename std::enable_if_t<
                details::And<details::Or<details::IsVoid<T>, details::NothrowConstructible<E, G&&>>, details::MoveAssignable<E>>::value,
                int> = 0>
        Expected& operator=(Unexpected<G>&& Unex) noexcept(details::NothrowConstructible<E, G&&>() && details::NothrowMoveAssignable<E>())
        {
            if (Super::HasValue())
            {
                if constexpr (!details::IsVoid<T>())
        ...
        template <
            typename U = T,
            typename std::enable_if_t<details::And<details::Not<details::IsVoid<T>>, details::AssignableFromU<T, E, U>>::value, int> = 0>
        Expected& operator=(U&& Value) noexcept(details::NothrowConstructible<T, U>() && details::NothrowAssignable<T&, U>())
        {
            if (Super::HasValue())
            {
    ...
    [[nodiscard]] constexpr bool operator==(const Expected<T1, E1>& X, const Expected<T2, E2>& Y) noexcept(
        noexcept(bool(*X == *Y) && bool(X.Error() == Y.Error())))
    {
        if (X.HasValue())
        {
    ...
    [[nodiscard]] constexpr bool operator==(const Expected<T1, E1>& X, const Unexpected<E2>& Unex) noexcept(noexcept(X.Error() == Unex.Value()))
    {
    ...
    template <
        ...
        typename std::enable_if_t<
            details::And<
                details::Or<details::IsVoid<T1>, details::And<details::MoveConstructible<T1>, details::Swappable<T1>>>,
                details::MoveConstructible<E1>,
                details::Swappable<E1>,
                details::Or<details::VoidOrNothrowMoveConstructible<T1>, details::NothrowMoveConstructible<E1>>>::value,
            int> = 0>
